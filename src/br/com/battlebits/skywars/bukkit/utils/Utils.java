...
import br.com.battlebits.skywars.bukkit.menu.KitInventory;
import br.com.battlebits.skywars.bukkit.menu.SpectatorMenu;
...
public class Utils {
    ...
    public static void addPlayerItems(Player player) {
        ...
        inventory.setItem(0, new ActionItemStack(new ItemBuilder().type(Material.ENDER_CHEST).name("§%skywars-kit-selector-item%§").build(), new InteractHandler() {
            @Override
        ...
        inventory.setItem(8, new ActionItemStack(new ItemBuilder().type(Material.WATCH).name("§%skywars-player-leave-item%§").build(), new InteractHandler() {
            public boolean onInteract(Player player, ItemStack item, Action action) {
